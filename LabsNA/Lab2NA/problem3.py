...
for line in lines:
    ...
    if time == ' Nan':
        ...
        print(f"For {days-1} items, this amount of time:{rounded_number} by Newton interp.")
        time = lagrange_interp(x1, y1, days - 1)
        print(f"For {days-1} items, this amount of time:{time} by Lagrange interp.")
        time = piecewise_linear_interp(x1, y1, days - 1)
        print(f"For {days-1} items, this amount of time:{time} by Piecewise interp.")
        time = cubic_spline_interp(x1, y1, days - 1)
        print(f"For {days-1} items, this amount of time:{time} by Cubic interp.")
        ...
        print(f"Area from 1 to {days-1} = {integral}")
        print("--------------------------------------------------------------")
    x1.append(int(items))
    y1.append(int(time))
    days += 1
